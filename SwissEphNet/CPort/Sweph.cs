...
namespace SwissEphNet.CPort
{
    ...
    partial class Sweph : BaseCPort
    {
        ...
        internal swe_data swed = new swe_data()
        {
            ...
            deps = null,   /* deps */
            timeout = 0,      /* timeout */
            astro_models = [0, 0, 0, 0, 0, 0, 0, 0,], /* astro_models */
        };
        ...
        static readonly string[] ayanamsa_name = [
        ...
        int[] pnoext2int = [SEI_SUN, SEI_MOON, SEI_MERCURY, SEI_VENUS, SEI_MARS, SEI_JUPITER, SEI_SATURN, SEI_URANUS, SEI_NEPTUNE, SEI_PLUTO, 0, 0, 0, 0, SEI_EARTH, SEI_CHIRON, SEI_PHOLUS, SEI_CERES, SEI_PALLAS, SEI_JUNO, SEI_VESTA,];
...
//}
            ...
            double[] xx, ref string serr)
        {
            ...
            if (ipl is < SwissEph.SE_NPLANETS and >= SwissEph.SE_SUN)
                sd = swed.savedat[ipl];
            else
        ...
        Int32 swecalc(double tjd, int ipl, Int32 iflag, CPointer<double> x, out string serr)
        {
            ...
            else if (ipl == SwissEph.SE_MOON)
            {
                ...
                switch (epheflag)
                {
                    case SwissEph.SEFLG_JPLEPH:
                        ...
                        {
                            if (tjd is > MOSHLUEPH_START and < MOSHLUEPH_END)
                            {
            ...
            else if (ipl == SwissEph.SE_SUN && (iflag & SwissEph.SEFLG_BARYCTR) != 0)
            {
                ...
                switch (epheflag)
                {
                    case SwissEph.SEFLG_JPLEPH:
                        ...
                        if (retc is ERR or BEYOND_EPH_LIMITS)
                        {
                            SE.SweJPL.swi_close_jpl_file();
                            swed.jpl_file_is_open = false;
                            goto return_error;
                    ...
                    sweph_sbar:
                        ...
                        //#if 1
                        if (retc is ERR or NOT_AVAILABLE)
                            goto return_error;
            ...
            else if (ipl == SwissEph.SE_OSCU_APOG)
            {
                ...
                if (tjd is < MOSHLUEPH_START or > MOSHLUEPH_END)
                {
                    for (i = 0; i < 24; i++)
                        x[i] = 0;
            ...
            || ipl > SwissEph.SE_AST_OFFSET)
            {
                ...
                if (ipli == SEI_CHIRON && tjd is < CHIRON_START or > CHIRON_END)
                {
                    serr = sprintf("Chiron's ephemeris is restricted to JD %8.1f - JD %8.1f",
                        CHIRON_START, CHIRON_END);
                    return ERR;
                }
                if (ipli == SEI_PHOLUS && tjd is < PHOLUS_START or > PHOLUS_END)
                {
                    serr = sprintf(
            ...
            do_asteroid:
                 ...
                 */
                if (retc is NOT_AVAILABLE or BEYOND_EPH_LIMITS)
                {
            ...
            else if (ipl is >= SwissEph.SE_FICT_OFFSET and <= SwissEph.SE_FICT_MAX)
            {
                //#if 0
                //       ipl == SE_CUPIDO
                //    || ipl == SE_HADES
        ...
        void load_dpsi_deps()
        {
            ...
            {
                cpos = s.Split([' '], StringSplitOptions.RemoveEmptyEntries);
                       ...
                       ref string serr)
        {
            ...
            switch (epheflag)
            {
                case SwissEph.SEFLG_JPLEPH:
                    ...
                    {
                        if (tjd is > MOSHPLEPH_START and < MOSHPLEPH_END)
                        {
                            iflag = (iflag & ~SwissEph.SEFLG_JPLEPH) | SwissEph.SEFLG_MOSEPH;
                            serr += " \nusing Moshier Eph; ";
                            goto moshier_planet;
        ...
        int sweph(double tjd, int ipli, int ifno, Int32 iflag, CPointer<double> xsunb, bool do_save, CPointer<double> xpret, ref string serr)
        {
            ...
            again:
                ...
                if (fdp.fptr == null)
                {
                    ...
                    if (ipli > SwissEph.SE_AST_OFFSET)
                    {
                        ...
                        {
                            s = sprintf("%ss.%s", s.Substring(0, sppi), SE_FILE_SUFFIX);
                            goto again;
                        }
                        /*
            ...
            {
                if (tjd < fdp.tfstart)
                    s = sprintf("jd %f < Swiss Eph. lower limit %f;",
                          tjd, fdp.tfstart);
                else
                    s = sprintf("jd %f > Swiss Eph. upper limit %f;",
                          tjd, fdp.tfend);
            ...
            if (do_save)
            {
                ...
                if (ifno is SEI_FILE_PLANET or SEI_FILE_MOON)
                    pdp.iephe = SwissEph.SEFLG_SWIEPH;/**/
                else
        ...
        public CFile swi_fopen(int ifno, string fname, string ephepath, ref string serr)
        {
            ...
            {
                s = cpos[i];
                fnamp = s.TrimEnd('\\', '/') + Path.DirectorySeparatorChar + fname;
                if (ifno >= 0)
                {
            ...
            serr = sprintf("SwissEph file '%s' not found in PATH '%s'", fname, ephepath);
            return null;
        }
        ...
        public Int32 swi_get_denum(Int32 ipli, Int32 iflag)
        {
            ...
            else if (ipli is SEI_CHIRON or SEI_PHOLUS or SEI_CERES or SEI_PALLAS or SEI_JUNO or SEI_VESTA)
            {
        ...
        int app_pos_etc_plan(int ipli, Int32 iflag, ref string serr)
        {
            ...
            if ((iflag & SwissEph.SEFLG_HELCTR) != 0)
            {
                if (pdp.iephe is SwissEph.SEFLG_JPLEPH or SwissEph.SEFLG_SWIEPH)
                    for (i = 0; i <= 5; i++)
                               ...
                               ref epsilon oe, ref string serr)
        {
            ...
            if ((iflag & SwissEph.SEFLG_SIDEREAL) != 0)
            {
                ...
                else
                {
                    ...
                    if (swi_get_ayanamsa_with_speed(pdp.teval, iflag, daya, ref serr) == ERR)
                        return ERR;
                    /* ... and restored */
                    for (i = 0; i < 24; i++)
                        pdp.xreturn[i] = xxsv[i];
                    pdp.xreturn[0] -= daya[0] * SwissEph.DEGTORAD;
                    pdp.xreturn[3] -= daya[1] * SwissEph.DEGTORAD;
        ...
        public void swe_set_sid_mode(Int32 sid_mode, double t0, double ayan_t0)
        {
            ...
            try
            {
                ...
                if (sid_mode is SwissEph.SE_SIDM_J2000 or SwissEph.SE_SIDM_J1900 or SwissEph.SE_SIDM_B1950 or SwissEph.SE_SIDM_GALALIGN_MARDYKS
                      )
                {
                    sip.sid_mode &= ~SwissEph.SE_SIDBIT_SSY_PLANE;
                    sip.sid_mode |= SwissEph.SE_SIDBIT_ECL_T0;
                }
                if (sid_mode is SwissEph.SE_SIDM_TRUE_CITRA or SwissEph.SE_SIDM_TRUE_REVATI or SwissEph.SE_SIDM_TRUE_PUSHYA or SwissEph.SE_SIDM_TRUE_SHEORAN or SwissEph.SE_SIDM_TRUE_MULA or SwissEph.SE_SIDM_GALCENT_0SAG or SwissEph.SE_SIDM_GALCENT_COCHRANE or SwissEph.SE_SIDM_GALCENT_RGILBRAND or SwissEph.SE_SIDM_GALCENT_MULA_WILHELM or SwissEph.SE_SIDM_GALEQU_IAU1958 or SwissEph.SE_SIDM_GALEQU_TRUE or SwissEph.SE_SIDM_GALEQU_MULA
                    )
        ...
        public Int32 swe_get_ayanamsa_ex(double tjd_et, Int32 iflag, out double daya, ref string serr)
        {
            ...
            Int32 retval = swi_get_ayanamsa_ex(tjd_et, iflag, out daya, ref serr);
            if (0 == (iflag & SwissEph.SEFLG_NONUT))
            {
        ...
        public Int32 swi_get_ayanamsa_ex(double tjd_et, Int32 iflag, out double daya, ref string serr)
        {
            ...
            if (swi_init_swed_if_start() == 1 && 0 == (epheflag & SwissEph.SEFLG_MOSEPH)
               && sip.sid_mode is SwissEph.SE_SIDM_TRUE_CITRA or SwissEph.SE_SIDM_TRUE_REVATI or SwissEph.SE_SIDM_TRUE_PUSHYA or SwissEph.SE_SIDM_TRUE_SHEORAN or SwissEph.SE_SIDM_TRUE_MULA or SwissEph.SE_SIDM_GALCENT_0SAG or SwissEph.SE_SIDM_GALCENT_COCHRANE or SwissEph.SE_SIDM_GALCENT_RGILBRAND or SwissEph.SE_SIDM_GALCENT_MULA_WILHELM or SwissEph.SE_SIDM_GALEQU_IAU1958 or SwissEph.SE_SIDM_GALEQU_TRUE or SwissEph.SE_SIDM_GALEQU_MULA
                && serr != null)
            {
        ...
        Int32 swi_get_ayanamsa_with_speed(double tjd_et, Int32 iflag, double[] daya, ref string serr)
        {
            double daya_t2, t2;
            double tintv = 0.001;
            Int32 retflag;
            t2 = tjd_et - tintv;
            retflag = swi_get_ayanamsa_ex(t2, iflag, out daya_t2, ref serr);
            if (retflag == ERR)
                return ERR;
            retflag = swi_get_ayanamsa_ex(tjd_et, iflag, out daya[0], ref serr);
            if (retflag == ERR)
                return ERR;
        ...
        public double swe_get_ayanamsa(double tjd_et)
        {
            ...
            swi_get_ayanamsa_ex(tjd_et, iflag, out daya, ref sdummy);
            return daya;
        }
        ...
        public double swe_get_ayanamsa_ut(double tjd_ut)
        {
            ...
            string sdummy = null;
            Int32 iflag = SE.SwephLib.swi_guess_ephe_flag();
            swi_get_ayanamsa_ex(tjd_ut + SE.swe_deltat_ex(tjd_ut, iflag, ref sdummy), 0, out daya, ref sdummy);
            return daya;
        }
        ...
        int app_pos_etc_plan_osc(int ipl, int ipli, Int32 iflag, ref string serr)
        {
            ...
            double[] xxsv = new double[6], xxsp = [0, 0, 0], xobs = new double[6], xobs2 = new double[6];
            double t;
            plan_data pdp = swed.pldat[ipli];
            plan_data pedp = swed.pldat[SEI_EARTH];
            plan_data psdp = swed.pldat[SEI_SUNBARY];
         ...
         */
        void aberr_light(CPointer<double> xx, CPointer<double> xe)
        {
            int i;
            double[] xxs = new double[6], v = new double[6], u = new double[6];
            double ru;
            double b_1, f1, f2;
            double v2;
            for (i = 0; i <= 5; i++)
        ...
        public void swi_deflect_light(CPointer<double> xx, double dt, Int32 iflag)
        {
            ...
            if (iephe is SwissEph.SEFLG_JPLEPH or SwissEph.SEFLG_SWIEPH)
                for (i = 0; i <= 2; i++)
        ...
        int get_new_segment(double tjd, int ipli, int ifno, ref string serr)
        {
            ...
            for (icoord = 0; icoord < 3; icoord++)
            {
                ...
                if (nco > pdp.ncoe)
                {
                    ...
                    serr = sprintf("error in ephemeris file %s: %d coefficients instead of %d. ", fdp.fnam, nco, pdp.ncoe);
                    //}
                    //free(pdp.segp);
                    pdp.segp = null;
                    return (ERR);
        ...
        int read_const(int ifno, ref string serr)
        {
            ...
            {
                serr = sprintf("Ephemeris file name '%s' wrong; rename '%s' ", sp, s);
                goto return_error;
            }
            /************************************* 
             * copyright                         *
            ...
            if (ifno == SEI_FILE_ANY_AST)
            {
                ...
                swed.ast_H = atof(s.Substring(35 + i));
                swed.ast_G = atof(s.Substring(42 + i));
        ...
        file_damage:
            ...
            serr = sprintf(serr_file_damage, fdp.fnam);
        //}
        //}
        return_error:
            fdp.fptr.Dispose();
        ...
        int do_fread(byte[] trg, int size, int count, int corrsize, ref CFile fp, Int32 fpos, int freord, int fendian, int ifno, ref string serr)
        {
            ...
            if (0 == freord && size == corrsize)
            {
                ...
                {
                    serr = sprintf("Ephemeris file %s is damaged (2).", swed.fidat[ifno].fnam);
                    return (ERR);
                }
                else
                    return (OK);
            ...
            else
            {
                ...
                {
                    serr = sprintf("Ephemeris file %s is damaged (4).", swed.fidat[ifno].fnam);
                    return (ERR);
                }
                if (size != corrsize)
                {
            ...
            for (j = 0; j <= 1; j++)
            {
                ...
                else
                {
                    if ((iflag & SwissEph.SEFLG_SIDEREAL) != 0)
                    {
                        ...
                        {
                            SE.SwephLib.swi_cartpol_sp(ndp.xreturn.GetPointer(6), ndp.xreturn);
                            if (swi_get_ayanamsa_with_speed(ndp.teval, iflag, daya, ref serr) == ERR)
                                return ERR;
                            ndp.xreturn[0] -= daya[0] * SwissEph.DEGTORAD;
                            ndp.xreturn[3] -= daya[1] * SwissEph.DEGTORAD;
  ...
  if (iflag & SEFLG_SIDEREAL) {
    ...
    oe = &swed.oec2000;
        ...
        Int32 plaus_iflag(Int32 iflag, Int32 ipl, double tjd, out string serr)
        {
            ...
            if (ipl is SwissEph.SE_OSCU_APOG or SwissEph.SE_TRUE_NODE or SwissEph.SE_MEAN_APOG or SwissEph.SE_MEAN_NODE or SwissEph.SE_INTP_APOG or SwissEph.SE_INTP_PERG)
                iflag = iflag & ~(SwissEph.SEFLG_JPLHOR | SwissEph.SEFLG_JPLHOR_APPROX);
            if (ipl is >= SwissEph.SE_FICT_OFFSET and <= SwissEph.SE_FICT_MAX)
                iflag = iflag & ~(SwissEph.SEFLG_JPLHOR | SwissEph.SEFLG_JPLHOR_APPROX);
         ...
         */
        Int32 save_star_in_struct(int nrecs, fixed_star fstp, ref string serr)
        {
            ...
            //  return ERR;
            //}
            var newfstars = new fixed_star[nrecs];
            if (swed.fixed_stars != null)
                swed.fixed_stars.CopyTo(newfstars, 0);
            swed.fixed_stars = newfstars;
            //ftarget = swed.fixed_stars + (nrecs - 1);
            //memcpy((void *) ftarget, (void *) fstp, sizestru);
            swed.fixed_stars[nrecs - 1] = fstp;
            return OK;
        }
        ...
        /*static int fstar_node_compare(fixed_star node1, fixed_star node2)
        {
        ...
        Int32 fixstar_cut_string(string srecord, ref string star, ref fixed_star stardata, ref string serr)
        {
            ...
            s = srecord;
            i = SwephLib.swi_cutstr(s, [','], out cpos, 20);
            /* return trad. name, nomeclature name */
            SwephLib.swi_right_trim(ref cpos[0]);
            SwephLib.swi_right_trim(ref cpos[1]);
            if (i < 14)
            {
                if (i >= 2)
                {
                ...
                {
                    //if (s.Length > 200) s[200] = '\0';
                    serr = sprintf("invalid line in fixed stars file: '%s'", s);
                }
                return ERR;
            }
        ...
        Int32 load_all_fixed_stars(ref string serr)
        {
            ...
            string srecord;
            fixed_star fstdata = new fixed_star();
            string last_starbayer = string.Empty;
            if (swed.n_fixstars_records > 0)
            {
                return -2;
            }
            ...
            while ((s = swed.fixfp.ReadLine()) != null)
            {
                ...
                fstdata.skey = sprintf(",%s", fstdata.starbayer); // , sorts before alnum
        ...
        Int32 fixstar_calc_from_struct(ref fixed_star stardata, double tjd, Int32 iflag, ref string star, CPointer<double> xx, ref string serr)
        {
            ...
            epheflag = iflag & SwissEph.SEFLG_EPHMASK;
            if (swi_init_swed_if_start() == 1 && 0 == (epheflag & SwissEph.SEFLG_MOSEPH) /*&& serr != NULL*/)
            {
            ...
            if ((iflag & SwissEph.SEFLG_TOPOCTR) != 0)
            {
                if (swi_get_observer(tjd - dt, iflag | SwissEph.SEFLG_NONUT, NO_SAVE, xobs_dt, ref serr) != OK)
                    return ERR;
                if (swi_get_observer(tjd, iflag | SwissEph.SEFLG_NONUT, NO_SAVE, xobs, ref serr) != OK)
                    return ERR;
                /* barycentric position of observer */
                for (i = 0; i <= 5; i++)
                {
            ...
            if ((iflag & SwissEph.SEFLG_TRUEPOS) == 0 && (iflag & SwissEph.SEFLG_NOABERR) == 0)
                swi_aberr_light_ex(x, xpo, xpo_dt, dt, iflag & SwissEph.SEFLG_SPEED);
            /* ICRS to J2000 */
            ...
            if ((iflag & SwissEph.SEFLG_SIDEREAL) != 0)
            {
                ...
                else
                {
                    ...
                    if (swi_get_ayanamsa_with_speed(tjd, iflag, daya, ref serr) == ERR)
                        return ERR;
                    x[0] -= daya[0] * SwissEph.DEGTORAD;
                    x[3] -= daya[1] * SwissEph.DEGTORAD;
                    SE.SwephLib.swi_polcart_sp(x, x);
                }
            }
            /************************************************
        ...
        Int32 search_star_in_list(ref string sstar, ref fixed_star stardata, ref string serr)
        {
            ...
            else if (sstar.Length > 0 && Char.IsDigit(sstar[0]))
            {
                star_nr = atoi(sstar);
            }
            else
            ...
            if (star_nr > 0)
            {
                ...
                {
                    //if (serr != NULL)
                    serr = sprintf("error, swe_fixstar(): sequential fixed star number %d is not available", star_nr);
                    return ERR;
                }
            ...
            else if (!is_bayer && (sp = sstar.IndexOf('%')) >= 0)
            {
                ...
                if (sp != sstar.Length - 1)
                {
                    //if (serr != NULL)
                    serr = sprintf("error, swe_fixstar(): invalid search string %s", sstar);
                    return ERR;
                }
                searchkey = sstar;
                len = sstar.Length;
                ...
                //if (serr != NULL)
                serr = sprintf("error, swe_fixstar(): star search string %s did not match", sstar);
                return ERR;
            ...
            else
            {
                ...
                stardatap = bsearch(searchkey, stardatabegp, ndata, fstar_node_compare);
                if (stardatap.IsNull)
                {
                    //if (serr != NULL)
                    serr = sprintf("error, swe_fixstar(): could not find star name %s", sstar);
                    return ERR;
                }
                stardata = stardatap[0];
        ...
        **********************************************************/
        ...
        public Int32 swe_fixstar2_ut(ref string star, double tjd_ut, Int32 iflag, double[] xx, ref string serr)
        {
            ...
            deltat = SE.swe_deltat_ex(tjd_ut, iflag, ref serr);
            /* if ephe required is not ephe returned, adjust delta t: */
            retflag = swe_fixstar2(ref star, tjd_ut + deltat, iflag, xx, ref serr);
            if (retflag != ERR && (retflag & SwissEph.SEFLG_EPHMASK) != epheflag)
            {
                deltat = SE.swe_deltat_ex(tjd_ut, retflag, ref sdummy);
                retflag = swe_fixstar2(ref star, tjd_ut + deltat, iflag, xx, ref sdummy);
            }
            return retflag;
        }
        ...
        **********************************************************/
        ...
        public string swe_get_planet_name(int ipl, ref string s)
        {
            //int i;
            //Int32 retc;
                    ...
                    if (ipl > SwissEph.SE_AST_OFFSET)
                    {
                        ...
                        else
                        {
                            ...
                            if (retc != ERR && retc != NOT_AVAILABLE)
                                ...
                                s = sprintf("%d: not found", ipl - SwissEph.SE_AST_OFFSET);
                        }
            ...
            //if (strlen(s) < 80)
            {
                ...
                strcpy(out swed.saved_planet_name, s);
            }
            return s;
        }
        ...
        void trace_swe_fixstar(int swtch, string star, double tjd, Int32 iflag, CPointer<double> xx, ref string serr)
        {
            //if (swi_trace_count >= TRACE_COUNT_MAX)
            //    return;
            switch (swtch)
        ...
        Int32 swi_fixstar_calc_from_record(string srecord, double tjd, Int32 iflag, ref string star, CPointer<double> xx, ref string serr)
        {
            ...
            double daya, rdist;
            double[] x = new double[6], xxsv = new double[6], xobs = new double[6], xobs_dt = new double[6];
            //, *xpo = NULL, *xpo_dt = NULL;
            CPointer<double> xpo, xpo_dt;
            ...
            if (epoch == 1950)
            {
                SE.SwephLib.swi_FK4_FK5(x, B1950);
                SE.SwephLib.swi_precess(x, B1950, 0, J_TO_J2000);
                SE.SwephLib.swi_precess((CPointer<double>)x + 3, B1950, 0, J_TO_J2000);
            }
            /* FK5 to ICRF, if jpl ephemeris is referred to ICRF.
             * With data that are already ICRF, epoch = 0 */
            if (epoch != 0)
            ...
            if ((iflag & SwissEph.SEFLG_EQUATORIAL) == 0)
            {
                SE.SwephLib.swi_coortrf2(x, x, oe.seps, oe.ceps);
                if ((iflag & SwissEph.SEFLG_SPEED) != 0)
                    SE.SwephLib.swi_coortrf2((CPointer<double>)x + 3, (CPointer<double>)x + 3, oe.seps, oe.ceps);
                if (0 == (iflag & SwissEph.SEFLG_NONUT))
                {
                    SE.SwephLib.swi_coortrf2(x, x, swed.nut.snut, swed.nut.cnut);
                    if ((iflag & SwissEph.SEFLG_SPEED) != 0)
                        SE.SwephLib.swi_coortrf2((CPointer<double>)x + 3, (CPointer<double>)x + 3, swed.nut.snut, swed.nut.cnut);
                }
            }
            /************************************
             * sidereal positions               *
            ...
            if ((iflag & SwissEph.SEFLG_SIDEREAL) != 0)
            {
                ...
                {
                    SE.SwephLib.swi_cartpol_sp(x, x);
                    if (swi_get_ayanamsa_ex(tjd, iflag, out daya, ref serr) == ERR)
                        return ERR;
        ...
        **********************************************************/
          ...
          CPointer<double> xx, ref string serr)
        {
            ...
            //static TLS char slast_stardata[AS_MAXCH];
            //static TLS char slast_starname[AS_MAXCH];
            string srecord; int sp;   /* 20 byte for SE_STARFILE */
            int retc;
            //if (serr != null)
            serr = string.Empty;
#if TRACE
            SE.SwephLib.swi_open_trace(out serr);
            trace_swe_fixstar(1, star, tjd, iflag, xx, ref serr);
#endif //* TRACE */
            retc = fixstar_format_search_name(star, ref sstar, ref serr);
            if (retc == ERR)
            ...
            {
                if ((sp = strchr(sstar, ',')) != -1) // cut off Bayer, if trad. name
                    //*sp = '\0';
                    sstar = sstar.Substring(0, sp);
            }
            /* star elements from last call: */
