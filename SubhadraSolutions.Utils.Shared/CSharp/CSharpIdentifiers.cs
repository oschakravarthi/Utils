...
namespace SubhadraSolutions.Utils.CSharp;
...
public static class CSharpIdentifiers
{
    ...
    public static IReadOnlyCollection<string> Keywords => _keywords;
    ...
    public static bool IsValidParsedIdentifier(string str)
    {
        ...
        if (!IsValidParsedIdentifierStart(str, 0))
        {
            return false;
        }
        ...
        {
            if (!IsValidParsedIdentifierPart(str, i))
            {
                return false;
            }
    ...
    {
        if (s[index] == '_' || (s[index] >= '0' && s[index] <= '9') || char.IsLetter(s, index))
        {
            return true;
        }
    ...
    {
        return s[index] == '_' || char.IsLetter(s, index) ||
               char.GetUnicodeCategory(s, index) == UnicodeCategory.LetterNumber;
    }

    public static bool TryParseRawIdentifier(string str, out string parsed)
    {
        if (string.IsNullOrEmpty(str) || _keywords.Contains(str))
        {
            parsed = null;
            return false;
        }
        ...
        var verbatimCharWidth = str[0] == '@' ? 1 : 0;

        for (var i = verbatimCharWidth; i < str.Length;) //Manual increment
        {
            var c = str[i];
            ...
            if (c == '\\')
            {
                ...
                int charCode;
                if (!TryParseHex(str.Substring(i + 2, charCodeLength), out charCode))
                {
                    parsed = null;
                    return false;
                }
                ...
                sb.Append(char.ConvertFromUtf32(
                    charCodeLength)); //Handle characters above 2^16 by converting them to a surrogate pair
                i += 2 + charCodeLength;
            }
